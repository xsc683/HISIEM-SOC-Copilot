...
flowchart TB
    ...
    subgraph NONPROD["非生产包（4）"]
        ...
        EVAL["evaluation<br/>评估预言机（28 文件）"]
...
| `evaluation` | 17 + 11 | cross_plane 5 / knowledge 6 | 评估预言机 |
...
| `evaluation` | 评估预言机（**28 文件**，**禁被生产层导入**） | `evaluation/oracle.py`、`evaluation/sealer.py` |
