...
| 包 | 是 | 不是 | 允许持有 |
| --- | --- | --- | --- |
| `evaluation` | **评估侧**：数据集物化 + 清单封存 | 生产层 | 场景与预期 |
...
**论断 4：「oracle 防火墙」是本篇最核心的机制**（`oracle.py` 只是评估包里的一个模块，「评估包」才是整包的规范名字）。
...
---
flowchart TB
    subgraph ORACLE["evaluation（评估侧，32 文件）"]
        SC["scenario_loader"]
        MAT["materializer"]
        INJ["injector"]
        VER["verifier"]
    ...
    FIREWALL["oracle 防火墙<br/>oracle / events / control events<br/>/ sealed object 永不进生产"]
