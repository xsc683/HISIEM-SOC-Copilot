...
### 3.1 治理模型：Discovery / Admission / Selection 是三件事

- **Discovery** —— 某个 server 声称自己提供什么。
- **Admission** —— 本系统决定信任什么：一条**手写的服务端条目**，声明内部名、受信描述、配置的 server、外部名、参数/结果契约、期望的 schema 指纹、风险分级、租户范围与逐能力边界。
- **Selection** —— 模型可以选什么：**只能是已准入的只读能力**。

三者不可互相替代：**可发现 ≠ 已准入 ≠ 可被模型选择**。

### 3.2 fail-closed 规则

| 规则 | 效果 |
| --- | --- |
| 未知 server | 拒绝 —— 只联系受信且已配置的端点 |
| 写 / 高风险能力 | `is_model_selectable` 要求 `READ_ONLY`；写能力**永不可被选择** |
| 未准入的动态工具 | 可被发现但未准入 —— 运维可见，**模型不可见** |
| Schema 漂移 | 对规范名 + 输入/输出 schema 做确定性 SHA-256 指纹；漂移 → `SCHEMA_MISMATCH` |
| 协议降级 | 固定生产协议版本；不支持/旧版协商被**拒绝**，而不是静默降级 |
| 租户 | 只用服务端受信上下文；**模型提供的 tenant 字段被拒绝** |

**结果处理顺序固定**：协议/结果类型 → 不支持的交互模式 → `is_error` → 受支持的内容类型 → 受信契约校验 → 全局与逐能力边界；失败映射为类型化类别。超限结果返回 `RESULT_TOO_LARGE`，**绝不静默截断**。

**所有远端内容按不可信数据处理**，包括看起来像指令的文本。

**`MCP V1 是只读的，模型可选写能力不存在`** —— 这是刻意的范围边界，不是待办特性。
